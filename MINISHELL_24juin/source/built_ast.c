...
/*   Updated: 2024/06/24 14:23:16 by bchapuis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ast_node *build_ast(t_token **tokens, bool debug)
{
    ...
    if (tokens[i]->type == TOKEN_REDIRECTION_IN && tokens[i + 1]->type == TOKEN_CMD && tokens[i + 2]->type == TOKEN_CMD)
    {
        if (debug)
    ...
    else if (tokens[i]->type == TOKEN_INSERTION && tokens[i + 1]->type == TOKEN_ARG && tokens[i + 2]->type == TOKEN_CMD)
    {
        if (debug)
