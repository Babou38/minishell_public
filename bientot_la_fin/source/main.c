...
/*   Updated: 2024/07/22 10:29:08 by babou            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_envlist(t_shell **shell, t_env_list **new_node, char *var, bool lone)
{
	int		j;
	bool	found;
	t_env_list *head;
...
int main (int argc, char **argv, char **envp)
{
	...
	ft_memset(&shell.cmd_list, 0, sizeof(t_command));
	...
	while(shell.exit == 0)
	{
		...
		if (shell.str != NULL)
		{
			...
			if (!is_valid)
				safe_exit(&shell, NULL, NONE, 1);
			process_message(&shell, shell.str);
			//print_tokens(shell.list_token);
			// shell.ast = build_ast(shell.list_token, false);
			// if (!shell.ast)
			// 	safe_exit(&shell, NULL, NONE, 1);
			// //printAST(shell.ast);
