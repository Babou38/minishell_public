...
void handle_command(t_shell *shell)
{
    ...
    {
        if (ft_strcmp(shell->ast->value, "exit") == 0)
            shell->exit = ft_is_exit(shell);
        else if (ft_strcmp(shell->ast->value, "cd") == 0)
            ft_cd(shell);
        else if (ft_strcmp(shell->ast->value, "pwd") == 0)
            ft_pwd(shell);
        else if (ft_strcmp(shell->ast->value, "echo") == 0)
            ft_echo(shell);
        else if (ft_strcmp(shell->ast->value, "env") == 0)
            ft_env(shell);
        else if (ft_strcmp(shell->ast->value, "unset") == 0)
            ft_unset(shell);
        else if (ft_strcmp(shell->ast->value, "export") == 0)
            ft_export(shell);
        else
