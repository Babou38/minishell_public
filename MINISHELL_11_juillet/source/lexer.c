...
char	*isolate_command(char *input, int *k, int i) //N
{
	...
	while (input[*k])
	{
		...
		if ((input[*k] == '|' || input[*k] == '<' || input[*k] == '>') && allow_space == false)
			break ;
		if (input[*k] == '\0' || (input[*k] == ' ' && allow_space == false))
			break ;
		(*k)++;
	}
	res = ft_strndup(input + i, *k - i);
	parsed_res = remove_first_quotes(res, 0, 0);
	free(res);
	return (parsed_res);
}
...
t_token **lexer(char *input, bool *is_valid) //N
{
	...
	while (input[i])
	{
		if (input[i] != ' ')
		{
			...
			{
				k = i;
				word = isolate_command(input, &k, i);  //NOUVEAU  //////////////////////////////////
				if (!word)
				{
					*is_valid = false;
					tokens[j] = NULL;
				...
				else
				{
					...
					if (j > 0 && (tokens[j - 1]->type == TOKEN_REDIRECTION_IN || tokens[j - 1]->type == TOKEN_INSERTION ||
                                  tokens[j - 1]->type == TOKEN_REDIRECTION_OUT || tokens[j - 1]->type == TOKEN_EXTRACTION))
                        tokens[j++] = create_token(TOKEN_ARG, word);
                    else
                        tokens[j++] = create_token(TOKEN_ARG, word);
				}
