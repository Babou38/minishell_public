...
{
    int i = 0;
    // printf("nouveaux ordre tokens\n");
    while (tokens[i] != NULL)
    {
        if (tokens[i]->type == TOKEN_CMD)
        {
            int j = i + 1;
            while (tokens[j] != NULL && tokens[j]->type != TOKEN_REDIRECTION_IN && tokens[j]->type != TOKEN_REDIRECTION_OUT &&
                   tokens[j]->type != TOKEN_INSERTION && tokens[j]->type != TOKEN_EXTRACTION)
            {
                j++;
            }

            if (tokens[j] != NULL && (tokens[j]->type == TOKEN_REDIRECTION_IN || tokens[j]->type == TOKEN_REDIRECTION_OUT ||
                                      tokens[j]->type == TOKEN_INSERTION || tokens[j]->type == TOKEN_EXTRACTION))
            {
                // Trouver la position du prochain pipe ou la fin des tokens
                int next_pipe = j + 2;
                while (tokens[next_pipe] != NULL && tokens[next_pipe]->type != TOKEN_PIPE)
                {
                    next_pipe++;
                }

                // Sauvegarder les tokens de redirection et de fichier
                t_token *redirection_token = tokens[j];
                t_token *file_token = tokens[j + 1];

                // Déplacer les tokens après la redirection pour combler le trou
                int k = j + 2;
                while (k < next_pipe)
                {
                    tokens[j] = tokens[k];
                ...
                // Insérer les tokens de redirection et de fichier juste avant le pipe ou à la fin
                tokens[j] = redirection_token;
                tokens[j + 1] = file_token;

                // Repositionner les autres tokens après les redirections insérées
                k = j + 2;
                while (tokens[next_pipe] != NULL)
                {
                    tokens[k] = tokens[next_pipe];
                    k++;
                    next_pipe++;
                }
                tokens[k] = NULL;
...
t_ast_node *build_ast(t_token **tokens, bool debug)
{
	...
	while (tokens[i] != NULL)
	{
		...
		else if (tokens[i]->type == TOKEN_CMD || tokens[i]->type == TOKEN_SINGLE_QUOTE || tokens[i]->type == TOKEN_DOUBLE_QUOTE)
		{
			free(node->value); 
			free(node);
			command_node = create_command_node(tokens, &i, node, debug);
