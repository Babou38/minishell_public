...
int	var_already_exists(t_shell *shell, int *i)
{
	while (shell->env[++(*i)])
	{
		if (ft_strcmp_c(shell->env[*i], shell->cmd[1], '=') == 0)
			return (1);
...
void	change_var_value(t_shell *shell, int i)
{
	...
	if (ft_strchr(shell->cmd[1], '=') == NULL) // si pas de = , on touche pas
		return ;
	//SINON :
	head = shell->list;
	target = get_var_name(shell->cmd[1]); //on prends le nom
	...
	if (head->value != NULL) // si on avait deja une valeur on la remplace
		free(head->value);
	head->value = get_var_value(shell->cmd[1]);
	free(shell->env[i]);
	shell->env[i] = ft_strdup(shell->cmd[1]); // change la var dans le char **
}
...
void ft_export(t_shell *shell)
{
	...
	i = -1;
	k = 0;
	if (shell->cmd[1] == NULL) // si juste "export"
	{
	...
	if (!var_already_exists(shell, &i)) // si la var existe pas : 
	{
		...
		add_envlist(&shell, &new_var, shell->cmd[1], false); // ajout liste chainee
	}
	else
