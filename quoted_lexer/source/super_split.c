							...
							unsigned int current_word, unsigned int *start)
{
	...
	while (current_word--)
	{
		while (s[i] && (char_sym(s[i], set)))
			i++;
		*start = i;
		while (s[i] && (is_in_quotes(s, (int)i)
				|| !char_sym(s[i], set)))
			i++;
	}
	return (i);
...
static char	*alloccc(const char *s, const char *set, unsigned int current_word)
{
	char			*word;
	unsigned int	start;
	unsigned int	end;
...
static unsigned int	count_word(const char *s, const char *charset)
{
	...
	while (s[i])
	{
		while (s[i] && (char_sym(s[i], charset)))
			i++;
		if (s[i] == '\'' || s[i] == '"')
			i++;
		if (s[i])
			count++;
		while (s[i] && (is_in_quotes(s, (int)i) || !char_sym(s[i], charset)))
			i++;
	}
	return (count);
}
...
char	**split_arguments(const char *s, char *set)
{
	...
	{
		array[current_word - 1] = alloccc(s, set, current_word);
